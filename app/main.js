...
// Load secrets/config from a local .env file (gitignored) so credentials are
// not hardcoded in tracked source. Zero-dependency parser: KEY=value lines,
// # comments and blank lines ignored, surrounding quotes stripped. Existing
// process.env values win, so CI/build-time env vars are never overwritten.
(function loadDotEnv() {
  for (const envPath of [path.join(__dirname, '.env'), path.join(__dirname, '..', '.env')]) {
    try {
      ...
      console.error('Failed to load .env at', envPath, err.message);
    }
  }
})();
